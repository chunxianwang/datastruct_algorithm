...
因为DFS是一种情况一种情况地的尝试，在把所有可能情况尝试完之前，无法确定哪个是最短，所以DFS必须把所有情况都找一遍，才能确定最终答案（DFS的优化就是剪枝，不剪枝很容易超时）。
而BFS从一开始就是尝试所有情况，所以只要找到第一个达到的那个点，那就是最短的路径，可以直接返回了，其他情况都可以省略了，所以这种情况下，BFS更高效。

问：BFS解法中的visited为什么可以全局使用？

答：BFS是在尝试所有的可能路径，哪个最快到达终点，哪个就是最短。那么每一条路径走过的路不同，visited（也就是这条路径上走过的点）也应该不同，那么为什么visited可以全局使用呢？
        ...
        while que:
            for _ in range(len(que)):
                ...
                for i, j in directions:
                    new_row = row + i
                    new_col = col + j
                    if 0 <= new_row < length and 0 <= new_col < length and grid[new_row][new_col] == 0 and not visited.get((new_row, new_col)):
                        if new_row == length - 1 and new_col == length - 1:
                            return path + 1
                        que.appendleft((new_row, new_col))
