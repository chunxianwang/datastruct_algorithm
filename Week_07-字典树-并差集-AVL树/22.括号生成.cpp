...
private:
    ...
    void addingpar(vector<string>& ans, string current, int num_addible_open, int num_addible_close) {
        if(num_addible_open == 0 && num_addible_close == 0) {
            ans.push_back(current);
            return;
        }
        if(num_addible_close > 0) addingpar(ans, current+")", num_addible_open, num_addible_close-1);
        if(num_addible_open > 0) addingpar(ans, current+"(", num_addible_open-1, num_addible_close+1);
    }
#endif
};
// @lc code=end
